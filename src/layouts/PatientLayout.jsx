...
export const PatientLayout = () => {
  ...
  const navItems = [
    { name: 'Today', path: '/patient/dashboard', icon: Activity },
    { name: 'Workout Arena', path: '/patient/workout', icon: Heart },
    { name: 'Progress Analytics', path: '/patient/analytics', icon: TrendingUp },
    { name: 'Recovery Journal', path: '/patient/journal', icon: BookOpen },
    { name: 'AI Coach Chat', path: '/patient/chat', icon: MessageSquare },
  ];
        ...
        <div className="relative border-b border-border-subtle p-3">
          ...
          >
            <div className="flex items-center gap-2 overflow-hidden text-left">
              <div className="flex items-center justify-center w-6 h-6 rounded bg-accent-indigo text-white font-bold text-xs shrink-0">
                P
              </div>
              {!collapsed && (
                <div className="truncate">
                  <h4 className="text-xs font-bold leading-none text-text-primary">Patient Workspace</h4>
                  <span className="text-[10px] text-text-muted">Active recovery</span>
                </div>
              )}
            </div>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all duration-150 ${
                  isActive 
                    ? 'bg-accent-indigo/10 text-accent-indigo font-medium border-l-2 border-accent-indigo rounded-l-none' 
                    : 'text-text-secondary hover:bg-bg-elevated hover:text-text-primary'
                }`}
              >
                <Icon size={18} className={isActive ? 'text-accent-indigo' : 'text-text-muted'} />
          ...
          {!collapsed && (
            <div className="flex items-center gap-2">
              ...
              <div className="text-left overflow-hidden">
                <p className="text-xs font-medium text-text-primary truncate">Alex Rivera</p>
                <p className="text-[10px] text-text-muted truncate">Knee Recovery</p>
              </div>
            </div>
          )}
          <button 
